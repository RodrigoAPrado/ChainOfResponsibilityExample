...
/**
 * Defines a handler for handling additions.
 */
public class AdditionHandler extends HandlerDefaults implements IHandler {

    /**
     * Tries to perform an addition. Passes request forward if it can't handle or throws exception.
    ...
    public double handle(IOperation request) {
        try{
            if(request.getOperationAction() != OperationAction.Add){
