...
/**
 * Defines a handler for handling multiplications.
 */
public class MultiplicationHandler extends HandlerDefaults implements IHandler {

    /**
     * Tries to perform a multiplication. Passes request forward if it can't handle or throws exception.
    ...
    public double handle(IOperation request) {
        try{
            if(request.getOperationAction() != OperationAction.Multiply){
